...
function mapDocument(
	...
	return {
		...
		/*
		 * SearchDocument is work-level metadata. Its ISBN/publisher
		 * arrays may mix many editions and languages, so do not use
		 * them as edition metadata here. Exact edition fields are
		 * only accepted through lookupExactIsbn().
		 */
		isbn: undefined,
		publisher: undefined,
...
async function lookupExactIsbn(
	...
	const editionLanguage = mapLanguage(
		edition.languages?.[0]?.key,
	);

	const preferredLanguage = hypothesis.hints.language
		?.slice(0, 2)
		.toLowerCase();
